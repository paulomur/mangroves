...
Now we can calculate different vegetation indices, add other ancillary information
and also import the basemap from which we want to collect the spectral data. The basemap that we are going to use is the map we created using S1+S2 which is a good representation of different classes for the period 2019-2020.
...
We use Image Segmentation SNIC algorithm for classification. For building the clusters we use MVI, it is performs well
for delineating mangroves extent. 
...
We also use 90 trees after tunning the numberofTrees. It seems 90 provides a more stable accuracy. 
