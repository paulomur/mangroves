...
.evaluate(function (composite_year) {
  ...
  var image = percentil
    ...
    Export.image.toAsset({
      ...
      assetId: 'SilviaCarbon_COL/Landsat_Predictors/Cienaga_Percentiles/percentiles_' + year, 
      crs: outProj,
      scale: 30,
      maxPixels: 1e13,
      region: featureCol,
...
Note: It is also possible to see <span style="color:red;"> RangeError: Maximum call stack size exceeded </span>.
Just click the Run button 2 or 3 times quickly. 
...
```{r eval=FALSE}
var mangroves = require('users/murillop/NASA_mangroves:mangroves');
...
However you can add your own indices once you save the main library ('users/murillop/NASA_mangroves:mangroves');
...
We mimimize the number of trees to optimize computation demand. We identify the number of trees using a simple tunnin available for more detail [**here.**](https://courses.spatialthoughts.com/end-to-end-gee.html#hyperparameter-tuning){target="_blank"}
...
We created a gif to visualize the dynamics of mangroves in Cienaga Grande. Our approach provides a quick reference to evaluate mangroves loss and gains. There is a high variability along time in this study region and further validation and cleaning the outputs is necessary.
